%% Run sample, self-selected parameters / spline controllers
% run muscle analysis and enable assistive moment
iSub=5;
iSynConf=2;

MRS=MRS_list{iSub,1,iSynConf};

Misc            =MRS{2}.Misc;
DatStore        =MRS{2}.DatStore;
Results_baseline=MRS{2}.Results;

Results_normal  =MRS{1}.Results;

% compute activation
clear MActivationUnassisted
acc=0;
for iTrial=1:Misc.nTrials
    MActivation=Results_baseline(iTrial).MActivation;
    data_length=length(MActivation);
    ind=[1:data_length]+acc;
    MActivationUnassisted(:,ind)=MActivation;
    acc=acc+data_length;
end
Misc.UnassistedActivations=MActivationUnassisted;
Misc.Advance.alpha=0.1;
Misc.Advance.ActivationLowerBound=1;

Misc.GetAnalysis = 0;
Misc.Advance.AssistiveDevice  = 1;
% Misc.Advance.ActivationLowerBound=1;
Misc.Advance.SynergyControl=1;

% con_list=[60 80 108 120 140];
con_list=[25 50 75 100 125 150];
nCon=length(con_list);

Results_assisted_array=cell(nCon,1);

ExecutionTime_1=datetime('now');
for iCon=1:length(con_list)

clear Device

for iTrial=1:nTrials
    Device{1}.Mode        = 'prescribed'; % opts: optimized and prescribed
    Device{1}.MuscleGroup = {'ankle_angle' -1};
    Device{1}.Type        = {'active' 'spline#N3'}; % opts: active, quasi-passive, passive, EMG-driven
    % Device{1}.Params      = [54.94 17.05 7.84 con_list(iCon)]; %54.8 14.5 8.1 51.8     108.06
    Device{1}.Params      = [54.86 27.45 7.00 con_list(iCon)]; %54.8 14.5 8.1 51.8     108.06

    [assistanceInfo]=generateTorque(Device{1},DatStore(1),Misc.time(iTrial,:),Misc.extra_frames);

    Device{1}.Assistance = assistanceInfo;

    Misc.Device(iTrial)=Device;
end

% to name and save results
Misc.to_save_results= 0;
Misc.OutName= 'example';

% time performance of a single loop
[Results_assisted,~,Misc]= MRS_Formulate_and_Solve_NM(Misc,DatStore);
Results_assisted_array(iCon)={Results_assisted};
end

ExecutionTime_2=datetime('now'); duration_loop=seconds(duration(ExecutionTime_2-ExecutionTime_1));
disp(['simulation took ' num2str(duration_loop,'%1.2f') ' secs']);
%%
clc;

assistiveGoal='eDot_MCLU24';

J_baseline_avg=zeros(nTrials,1);
J_baseline_TS_resampled = zeros(nTrials, 101);

for iTrial=1:nTrials
    [J_baseline_avg(iTrial),J_baseline_TS,J_baseline_extra] = computeOuterLoopFunction(Misc,Results_baseline(iTrial),assistiveGoal);

    original_x = linspace(0, 1, length(J_baseline_TS));
    new_x = linspace(0, 1, 101);
    J_baseline_TS_resampled(iTrial, :) = interp1(original_x, J_baseline_TS, new_x, 'linear');
end

J_example_TS_resampled = zeros(nTrials, nCon,101);
J_example_avg=zeros(nTrials,nCon);
J_per        =zeros(nTrials,nCon);

for iTrial=1:nTrials
    for iCon=1:nCon
        [J_example_avg(iTrial,iCon),J_example_TS] = computeOuterLoopFunction(Misc,Results_assisted_array{iCon}(iTrial),assistiveGoal);

        original_x = linspace(0, 1, length(J_example_TS));
        new_x = linspace(0, 1, 101);
        J_example_TS_resampled(iTrial, iCon, :) = interp1(original_x, J_example_TS, new_x, 'linear');

        J_per(iTrial,iCon)=(J_example_avg(iTrial,iCon)-J_baseline_avg(iTrial))/J_baseline_avg(iTrial)*100;
        disp(['goal change: ' num2str(J_per(iTrial,iCon),'%+1.2f') '%'])
    end
end
%%
color_list = {'#f9c1c5' '#f29ca3' '#e86975' '#cc2525' '#a21920' '#78080d'};
% Check results
to_plot=1;
if to_plot==1

    to_plot_act=1;
    if to_plot_act==1
        figure; clf; % muscle activations
        for i=1:40
            subplot(5,8,i)

            for iTrial=1:nTrials
                hold on
                plot(Results_normal(iTrial).MActivation(i,:),'k')
                plot(Results_baseline(iTrial).MActivation(i,:),'--g')

                for iCon=1:nCon
                    plot(Results_assisted_array{iCon}(iTrial).MActivation(i,:),'LineStyle','-','Color',color_list{iCon},'LineWidth',2)
                end
            end

            ylim([0 1])
            title(Results_assisted(1).MuscleNames{i})

        end
        sgtitle('muscle activations')
    end

    to_plot_syn=0;
    if to_plot_syn==1
        figure; clf; % muscle activations
        N=Misc.SynCon.N;
        for i=1:N
            subplot(2,3,i)
            hold on
            for iTrial=1:nTrials
                plot(Results_baseline(iTrial).SynergyControl.H(i,:),'b','LineWidth',3)
            end

            for iTrial=1:nTrials
                for iCon=1:nCon
                    plot(Results_assisted_array{iCon}(iTrial).SynergyControl.H(i,:),'LineWidth',3,'LineStyle','-','Color',color_list{iCon})
                end
            end

            ylim([0 1])
        end
    end

    % figure; clf; % device torque
    % nDevs=length(Device);
    % norMass=Misc.subject_data.subject_mass;
    % for iDev=1:nDevs
    %     gaitCycle=Device{iDev}.Assistance.Profile.GaitCycle;
    %     iDOF=strcmp(DatStore.DOFNames,Device{iDev}.MuscleGroup{1});
    %     sID =DatStore.IDinterp(:,iDOF)*Device{iDev}.MuscleGroup{2};
    %     Torque=Device{iDev}.Assistance.Profile.Torque;
    %     subplot(nDevs,1,iDev)
    %     plot(gaitCycle,sID/norMass,'k'); hold on
    %     plot(gaitCycle,Torque/norMass,'r')
    %     title([Device{iDev}.MuscleGroup{1}],'Interpreter','none')
    % end
    % sgtitle('assistive torque')

    clear p;

    to_plot_eDot=1;
    if to_plot_eDot==1
        figure(10); clf;


        for iTrial=1:nTrials
            subplot(2,2,1); hold on
            p(iTrial)=plot(J_baseline_TS_resampled(iTrial,:),'LineWidth',3,'LineStyle','-','Color','k','DisplayName',[' Baseline (avg value) = ' num2str(J_baseline_avg(iTrial),'%1.1f')  ' ' J_baseline_extra.unit]);
        end
        legend(p);
        xlabel('gait cycle [%]'); ylabel([J_baseline_extra.label ' [' J_baseline_extra.unit ']']);
        ylim([0 6])

        for iTrial=1:nTrials
            subplot(2,3,3+iTrial); hold on
            for iCon=1:nCon
                p(iCon)=plot(squeeze(J_example_TS_resampled(iTrial,iCon,:)),'LineWidth',3,'LineStyle','-','Color',color_list{iCon},'DisplayName',[' Assisted (avg value) = ' num2str(J_example_avg(iTrial,iCon),'%1.1f')  ' ' J_baseline_extra.unit ...
                    ' - change: ' num2str(J_per(iTrial,iCon),'%+1.1f') '%']);
            end
            % legend(p);
            xlabel('gait cycle [%]'); ylabel([J_baseline_extra.label ' [' J_baseline_extra.unit ']']);
            ylim([0 6])
        end

        subplot(2,2,2); hold on
        for iTrial=1:nTrials
            for iCon=1:nCon
                plot(con_list(iCon),J_per(iTrial,iCon),'Marker','o','MarkerFaceColor',color_list{iCon},'MarkerSize',7,'MarkerEdgeColor','k')
            end
        end
        xlabel('peak torque [Nm]'); ylabel('metabolic reduction [%]');
        axis([0 150 -30 5])
    end

    % title([ 'baseline (avg value) = ' num2str(J_baseline_avg,'%1.1f') ' ' J_baseline_extra.unit ...
    %        ' assisted (avg value) = ' num2str(J_example_avg,'%1.1f')  ' ' J_baseline_extra.unit ...
    %        ' - change: ' num2str(J_per,'%+1.1f') '%']);
end


%% Run sample, self-selected parameters / spline controllers
% run muscle analysis and enable assistive moment
Misc.GetAnalysis = 0;
Misc.Advance.AssistiveDevice  = 1;

% NOTE: Available assistance
% anklePlantarflexion: ankle_angle_   -1
% kneeExtension:       knee_angle_    -1
% hipFlexion:          hip_flexion_    1
% hipAbduction:        hip_adduction_ -1

clear Device
select_simulation_type='active spline#N3';
if strcmp(select_simulation_type,'active spline#N3')
    Device{1}.Mode        = 'prescribed'; % opts: optimized and prescribed
    Device{1}.MuscleGroup = {['ankle_angle_' Misc.gait_data.side_sel] -1};
    Device{1}.Type        = {'active' 'spline#N3'}; % opts: active, quasi-passive, passive, EMG-driven
    Device{1}.Params      = [54.1 22 8.6 77.9]; %54.8 14.5 8.1 51.8
    [assistanceInfo]=generateTorque(Device{1},DatStore,Misc.time,Misc.extra_frames);

    Device{1}.Assistance = assistanceInfo;
elseif strcmp(select_simulation_type,'quasi-passive clutchSpring')
    % generate DeviceSetup
    Device{1}.Mode        = 'prescribed';    % opts: optimized and prescribed
    % Device{1}.MuscleGroup = {['hip_adduction_' Misc.gait_data.side_sel] -1}; 
    % Device{1}.MuscleGroup = {['hip_flexion_' Misc.gait_data.side_sel] 1}; 
    Device{1}.MuscleGroup = {['knee_angle_' Misc.gait_data.side_sel] -1};    
    % Device{1}.MuscleGroup = {['ankle_angle_' Misc.gait_data.side_sel] -1};  
    Device{1}.Type        = {'quasi-passive' 'clutchSpring'}; % opts: active, quasi-passive, passive, EMG-driven
    Device{1}.Params      = [2.99 2.872];
    [assistanceInfo]=generateTorque(Device{1},DatStore,Misc.time,Misc.extra_frames);

    Device{1}.Assistance = assistanceInfo;
    Misc.Device=Device;
elseif strcmp(select_simulation_type,'double')
    % DEVICE 1
    Device{1}.Mode        = 'prescribed'; % opts: optimized and prescribed
    Device{1}.MuscleGroup = {['knee_angle_' Misc.gait_data.side_sel] -1};
    Device{1}.Type        = {'active' 'spline#N3'}; % opts: active, quasi-passive, passive, EMG-driven
    Device{1}.Params      = [12.4 6.3 24 67.3];
    [assistanceInfo]=generateTorque(Device{1},DatStore,Misc.time,Misc.extra_frames);

    Device{1}.Assistance = assistanceInfo;

    % DEVICE 2
    Device{2}.Mode        = 'prescribed'; % opts: optimized and prescribed
    Device{2}.MuscleGroup = {['ankle_angle_' Misc.gait_data.side_sel] -1};
    Device{2}.Type        = {'active' 'spline#N3'}; % opts: active, quasi-passive, passive, EMG-driven
    Device{2}.Params      = [39.2 20.2 20.9 45.1];
    [assistanceInfo]=generateTorque(Device{2},DatStore,Misc.time,Misc.extra_frames);

    Device{2}.Assistance = assistanceInfo;    
end
Misc.Device=Device;

% to name and save results
Misc.to_save_results= 0;
Misc.OutName= 'example';

% time performance of a single loop
ExecutionTime_1=datetime('now');
[Results_assisted,~,Misc]= MRS_Formulate_and_Solve_NeuroCons(Misc,DatStore);
ExecutionTime_2=datetime('now'); duration_loop=seconds(duration(ExecutionTime_2-ExecutionTime_1));
disp(['simulation took ' num2str(duration_loop,'%1.2f') ' secs']);

[J_example_avg,J_example_TS,J_example_extra] = computeOuterLoopFunction(Misc,Results_assisted,assistiveGoal);
J_per=(J_example_avg-J_baseline_avg)/J_baseline_avg*100;
disp(['goal change: ' num2str(J_per,'%+1.2f') '%'])

% Check results
to_plot=1;
if to_plot==1
    figure; clf; % muscle activations
    for i=1:40
        subplot(5,8,i)
        hold on
        plot(Results_normal.MActivation(i,:),'k')
        plot(Results_baseline.MActivation(i,:),'--g')
        plot(Results_assisted.MActivation(i,:),':r')
        ylim([0 1])
        title(Results_assisted.MuscleNames{i})

    end
    sgtitle('muscle activations')

    figure; clf; % device torque
    nDevs=length(Device);
    norMass=Misc.subject_data.subject_mass;
    for iDev=1:nDevs
        gaitCycle=Device{iDev}.Assistance.Profile.GaitCycle;
        iDOF=strcmp(DatStore.DOFNames,Device{iDev}.MuscleGroup{1});
        sID =DatStore.IDinterp(:,iDOF)*Device{iDev}.MuscleGroup{2};
        Torque=Device{iDev}.Assistance.Profile.Torque;
        subplot(nDevs,1,iDev)
        plot(gaitCycle,sID/norMass,'k'); hold on
        plot(gaitCycle,Torque/norMass,'r')
        title([Device{iDev}.MuscleGroup{1}],'Interpreter','none')
    end
    sgtitle('assistive torque')

    figure; clf;
    plot(J_baseline_TS,'k','LineWidth',2); hold on
    plot(J_example_TS,'r','LineWidth',2)
    xlabel('gait cycle[%]'); ylabel([J_example_extra.label ' [' J_example_extra.unit ']']);
    title([ 'baseline (avg value) = ' num2str(J_baseline_avg,'%1.1f') ' ' J_example_extra.unit ...
           ' assisted (avg value) = ' num2str(J_example_avg,'%1.1f')  ' ' J_example_extra.unit ...
           ' - change: ' num2str(J_per,'%+1.1f') '%']);
end