...
if strcmp(computationCase,'all')
    % synergy analysis
    [W,H,Htrial,~,synMetrics]=synergyAnalysis(Results_noSyn,synergy_list,[0 0 0]);
    save(fullfile(SynergyPath,'synergy_metrics.mat'),'synMetrics');

    % Misc.MotionSelection
    sTrial=str2double(Misc.MotionSelection(end));

    % compute for each synergy number
    for iSyn=1:length(synergy_list)
        sSyn=synergy_list(iSyn);
        Wsel=W{iSyn};
        Hsel=Htrial{sTrial,iSyn};
