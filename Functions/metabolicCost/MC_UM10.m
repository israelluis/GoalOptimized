function[MC_parameter,E_rate,w_rate,h_rate]= MC_UM10(muscle_parameter,muscle_DynCon,time,option)
% read inputs
...
for i=1:data_length
   if L_CE(i)<=OFL
   h_AM_rate_scaled(i,1)= h_AM_rate(i).*A_AM(i).*S*m;
   elseif L_CE(i)>OFL
   h_AM_rate_scaled(i,1)= (0.4*h_AM_rate(i)+0.6*h_AM_rate(i).*fl_act_multiplier(i)).*A_AM(i).*S*m;
   end
end
...
for i=1:data_length
   if L_CE(i)<=OFL
   h_SL_rate_scaled(i,1)= h_SL_rate(i)*S*m;
   elseif L_CE(i)>OFL
   h_SL_rate_scaled(i,1)= h_SL_rate(i).*fl_act_multiplier(i)*S*m;
   end
end
%% Work rate
...
for i=1:data_length
   if VCEtilde(i)<=0
      w_rate(i,1) = -muscle_fiber_velocity(i).*F_CE(i);
   elseif VCEtilde(i)>0
      w_rate(i,1) =  0;
   end
end
