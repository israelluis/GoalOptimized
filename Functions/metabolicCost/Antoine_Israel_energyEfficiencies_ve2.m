...
muscle_parameter.OFL = Misc.params(2,1);
...
% muscle_parameter.mass=0.2662; % antoine mass
...
[MC_parameter_UM03_0,~,~,~]= MC_UM03_R_published(muscle_parameter,muscle_DynCon,0,basalOn);
[MC_parameter_UM10_0,~,~,~]= MC_UM10_R_published(muscle_parameter,muscle_DynCon,0,basalOn);
[MC_parameter_UC16_0,~,~,~]= MC_UC16_R_prior(muscle_parameter,muscle_DynCon,0,basalOn);
[MC_parameter_BH04_0,~,~,~]= MC_BH04_R_published(muscle_parameter,muscle_DynCon,0,basalOn);

Results.params.genericMRS(1,1)=muscle_parameter.MIF;
Results.params.genericMRS(2,1)=muscle_parameter.OFL;
Results.params.genericMRS(3,1)=0;
Results.params.genericMRS(4,1)=0;
Results.params.genericMRS(5,1)=muscle_parameter.MCV;
Results.params.genericMRS(6,1)=0;
Results.params.genericMRS(7,1)=0;
Results.params.genericMRS(8,1)=0;

clear Results.TForce.genericMRS
Results.TForce.genericMRS=ones(1,101)*0; % i dont not use this

[musT_param,musE_param,mus_time,mus_dyn,states_field] = Results_states_params(Results,opt.subjects_mass(sub_sel),opt.subjects_height(sub_sel),0);
mus_Dynamics= [mus_time; squeeze(mus_dyn)];

[MC_parameter_BH04_new,~,~,~]= MC_BH04_R(musT_param,musE_param,mus_Dynamics,mode_negWork,basalOn);
[MC_parameter_UM10_new,~,~,~]= MC_UM10_R(musT_param,musE_param,mus_Dynamics,mode_negWork,basalOn);

% NOTE:
% - BHARGAVA
% IF SAME MASS AND LM=LMTILDE AND DECAY=1 AS ANTOINE, THEN WE HAVE THE SAME
% RESULTS. IF I USE MY MASS WITHOUT LM AND DECAY UPDATE, WE HAVE VERY
% SIMILAR RESULTS ANYWAY
% - OTHER MODELS (e.g., UM10)
% MASS IS NOT THE SAME, THE CHANGES RESULTS. MINE HAS LOWER EFFICIENCY THAN
% ANTOINE FOR SOLEUS
%%
clc;
MC_parameter    =MC_parameter_UM10_0;
MC_parameter_new=MC_parameter_UM10_new;
title_MRS_MC = {'Umb2010'}; % Uch2016 Umb2003 Bargh2004
figure(2);

% MC_parameter=MC_parameter_BH04_0; 
% MC_parameter_new=MC_parameter_BH04_new;
% title_MRS_MC = {'Bargh2004'}; % Uch2016 Umb2003 Bargh2004
% figure(2);
...
clf;
subplot(2,2,1);
plot(E.genericMRS.([title_MRS_MC{:}]).Edot,'k','LineWidth',2); hold on;
plot(MC_parameter(1,:),':r','LineWidth',2);
plot(MC_parameter_new(1,:),'--b','LineWidth',4);
title({'energy rate'});

subplot(2,2,2);
plot(E.genericMRS.([title_MRS_MC{:}]).E_am,'k','LineWidth',2); hold on;
plot(MC_parameter(4,:),':r','LineWidth',2);
plot(MC_parameter_new(2,:),'--b','LineWidth',4);
plot(E.genericMRS.([title_MRS_MC{:}]).Wdot,'k','lineWidth',2); hold on;
plot(MC_parameter(2,:),':r','lineWidth',2);
plot(MC_parameter_new(4,:),'--b','LineWidth',4);
title({'E_AM and work'})

subplot(2,2,3);
plot(E.genericMRS.([title_MRS_MC{:}]).E_sl,'k','LineWidth',2); hold on;
plot(MC_parameter(5,:),':r','LineWidth',2);
plot(MC_parameter_new(5,:),'--b','LineWidth',4);
title({'E_SL'})

subplot(2,2,4);
plot(E.genericMRS.([title_MRS_MC{:}]).Wdot./E.genericMRS.([title_MRS_MC{:}]).Edot,'k','LineWidth',2); hold on;
plot(MC_parameter(2,:)./MC_parameter(1,:),':r','LineWidth',2);
title({'efficiency'})
%%
