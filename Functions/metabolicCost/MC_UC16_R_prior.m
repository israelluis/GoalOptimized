function[MC_parameter,E_rate,w_rate,h_rate]= MC_UC16_R_prior(muscle_parameter,muscle_DynCon,option,basalOn)
% read inputs
Mft= muscle_parameter.FT;      % from 0 to 1
lMo = muscle_parameter.OFL;
vMmax = muscle_parameter.MCV;
FMo = muscle_parameter.MIF;                 
PCSA= muscle_parameter.PCSA;
Mmass= muscle_parameter.mass;

MExcitation= muscle_DynCon.muscle_excitation;
MActivation= muscle_DynCon.muscle_activation;
MTUforce  = muscle_DynCon.muscle_MTUforce;     
Vce      = muscle_DynCon.V_CE; %+shortening -lengthening
Wdotce      = muscle_DynCon.W_CE; % W_CE= F_CE*V_CE: +W_CE spent -W_CE gain

lMtildeopt  = muscle_DynCon.lMtilde;
FMltilde  = muscle_DynCon.vMtilde;
fl_act_multiplier=muscle_DynCon.fl_act_multiplier(:);
FMvtilde   =muscle_DynCon.f_v_multiplier(:);
